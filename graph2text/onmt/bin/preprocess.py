...
def process_one_shard(corpus_params, params):
    corpus_type, fields, graph_reader, src_reader, tgt_reader, plan_reader, tree_reader, align_reader, opt,\
         existing_fields, src_vocab, tgt_vocab = corpus_params
    i, (graph_shard, src_shard, tgt_shard, plan_shard, tree_shard, align_shard, maybe_id, filter_pred) = params
    # create one counter per shard
    sub_sub_counter = defaultdict(Counter)
    assert len(src_shard) == len(tgt_shard) == len(graph_shard) == len(plan_shard) == len(tree_shard)
    ...
    plan_data = {"reader": plan_reader, "data": plan_shard, "dir": None}
    tree_data = {"reader": tree_reader, "data": tree_shard, "dir": None}
    align_data = {"reader": align_reader, "data": align_shard, "dir": None}
    _readers, _data, _dir = inputters.Dataset.config(
        [('graph', graph_data), ('src', src_data), ('tgt', tgt_data), 
         ('plan', plan_data), ('tree', tree_data), ('align', align_data)])
...
def build_save_dataset(corpus_type, fields, src_reader, graph_reader, tgt_reader,
                       plan_reader, tree_reader, align_reader, opt):
    ...
    if corpus_type == 'train':
        ...
        tgts = [opt.valid_tgt]
        plans = [opt.valid_plan]
        trees = [opt.valid_tree]
        ids = [None]
        aligns = [opt.valid_align]

    # Nothing, no vocab files
    ...
    def shard_iterator(graphs, srcs, tgts, plans, trees, ids, aligns, existing_shards,
                       existing_fields, corpus_type, opt):
        """
        Builds a single iterator yielding every shard of every corpus.
        """
        for graph, src, tgt, plan, tree, maybe_id, maybe_align in zip(graphs, srcs, tgts, plans, trees, ids, aligns):
            ...
            plan_shards = split_corpus(plan, opt.shard_size)
            tree_shards = split_corpus(tree, opt.shard_size)
            align_shards = split_corpus(maybe_align, opt.shard_size)
            for i, (gs, ss, ts, ps, trs, a_s) in enumerate(
                    zip(graph_shards, src_shards, tgt_shards, plan_shards, tree_shards, align_shards)):
                yield (i, (gs, ss, ts, ps, trs, a_s, maybe_id, filter_pred))

    shard_iter = shard_iterator(graphs, srcs, tgts, plans, trees, ids, aligns, existing_shards,
                                existing_fields, corpus_type, opt)

    with Pool(opt.num_threads) as p:
        dataset_params = (corpus_type, fields, graph_reader, src_reader, tgt_reader,
                          plan_reader, tree_reader,
...
def preprocess(opt):
    ...
    plan_reader = inputters.str2reader["text"].from_opt(opt)
    tree_reader = inputters.str2reader["text"].from_opt(opt)
    align_reader = inputters.str2reader["text"].from_opt(opt)

    logger.info("Building & saving training data...")
    build_save_dataset(
        'train', fields, src_reader, graph_reader, tgt_reader, plan_reader, tree_reader, align_reader, opt)
    if opt.valid_src and opt.valid_tgt:
        logger.info("Building & saving validation data...")
        build_save_dataset(
            'valid', fields, src_reader, graph_reader, tgt_reader, plan_reader, tree_reader, align_reader, opt)
