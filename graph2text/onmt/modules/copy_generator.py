...
class CopyGeneratorPlan(nn.Module):
    """Copy (Pointer) Generator for generating the plan"""

    def __init__(self, input_size, output_size=4, pad_idx=3, start_doc_idx=1):
        """
        Output size is the 4 special tokens from the plan:
            -1: end edu
            -2: begining of a document
            -3: end of a document (stop producing edus)
            -4: padding token for batches
        Important: during prediction if the generator decides to
        copy a word from the input (the 4 special tokens), -2 and -4
        will be masked out (we're interested only in end edus and end document)

        Input size is the hidden layer at each pass from special
        token/node (since it's copy generator)
        """
        super(CopyGeneratorPlan, self).__init__()
    ...
    def forward(self, hidden, attn):
        ...
        logits[:, self.start_doc_idx] = -float('inf')
...
class CopyGeneratorLoss(nn.Module):
    ...
    def forward(self, scores, align, target):
        ...
        #print("TARGET in LOSS: ", target.shape, target)
        #print("ALIGN in Loss: ", align.shape, align)
        #print("VOCAB SIZE", self.vocab_size)
        vocab_probs = scores.gather(1, target.unsqueeze(1)).squeeze(1)

        # probability of tokens copied from source
        copy_ix = align.unsqueeze(1) + self.vocab_size
        #print("SCORES BEFORE ", scores.shape, scores)
        copy_tok_probs = scores.gather(1, copy_ix).squeeze(1)
        #print("SCORES AFTER ", copy_tok_probs.shape, copy_tok_probs) 
        copy_tok_probs[align == self.unk_index] = 0
        copy_tok_probs += self.eps  # to avoid -inf logs
        #print("FORCE COPY ", self.force_copy)
...
class CopyGeneratorLossCompute(NMTLossCompute):
    ...
    def _compute_loss(self, batch, output, target, copy_attn, align,
        ...
        """
        #print("PLAN DECODER: ", output.shape, output)
        #print("PLAN Target: ", target.shape, target)
        #print("ATTENTION: ", copy_attn, copy_attn['std'].shape, copy_attn['std'])
        target = target.view(-1)
        align = align.view(-1)
        copy_attn = copy_attn['std']
        # print("TODO: Normalize by length copy loss")
