...
from onmt.decoders.dependency_tree_rel_model import DependencyTreeRelModel


def build_embeddings(opt, text_field, model_part):
    """
    Args:
    ...
    """
    emb_dim = opt.src_word_vec_size if model_part == "encoder" else opt.tgt_word_vec_size
    ...
    if model_part == "plan_decoder":
        word_padding_idx, feat_pad_indices = 3, []
        num_word_embeddings, num_feat_embeddings = 4, []
    else:
    ...
    fix_word_vecs = opt.fix_word_vecs_enc if model_part == "encoder" \
...
def build_base_model(model_opt, fields, gpu, checkpoint=None, gpu_id=None):
    ...
    if model_opt.model_type == "text" or model_opt.model_type == "vec":
        ...
        src_emb = build_embeddings(model_opt, src_field, model_part="encoder")
    else:
        src_emb = None
    # Build encoder.
    ...
    tgt_emb = build_embeddings(model_opt, tgt_field, model_part="decoder")
    ...
    plan_emb = build_embeddings(model_opt, plan_field, model_part="plan_decoder")
    plan_decoder = build_decoder(model_opt, plan_emb, is_plan=True)
    
    
    # Build tree decoder
    if model_opt.use_tree:
        tree_decoder = DependencyTreeRelModel(model_opt)
    else:
        tree_decoder = None
    
    # Build NMTModel(= encoder + decoder + plan + tree).
    ...
    model = onmt.models.NMTPlanModel(encoder, decoder, plan_decoder, tree_decoder)
    
    # Build Plan Generator
    vocab_size = 4
    pad_idx = 3
    start_doc_idx = 1
    # plan_generator = CopyGeneratorPlan(model_opt.dec_rnn_size, vocab_size, pad_idx, start_doc_idx)
    ...
    if checkpoint is not None:
        ...
        if model_opt.param_init != 0.0:
            ...
            for p in plan_generator.parameters():
                p.data.uniform_(-model_opt.param_init, model_opt.param_init)
    ...
    model.plan_generator = plan_generator
    model.to(device)
